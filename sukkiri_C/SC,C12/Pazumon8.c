...
int calcAttackDamage (Field* pField,Element element, int Length, int Combo) {
    ...
    int damage = (int)((AttackMonster.Attack*AttackBoost - pField->pEneMonster->Gard) *(1.5*zyou)*blurDamage(10)) ;
    if (damage<=0){
        damage =1;
    }
...
Field* doRecover(Field* pField, int Length, int Combo) {
    int calcRHP = 20*1.5*(Length - 3 + Combo)*blurDamage(10) ;
    int RecoverHP ;
...
int calcEnemyAttackDamage (Field* pField) {
    int damage = (int)(pField->pEneMonster->Attack - pField->pParty->PartyGard)*blurDamage(10) ;
    if (damage <= 0){
        damage =1;
    }
    return damage ;
