...
Field* doAttack ( Field* pField,Element element,int Length ,int Combo) ;         //味方の1回の攻撃について
int calcAttackDamage (Field* pField,Element element, int Length, int Combo) ;
Field* doRecover(Field* pField, int Length, int Combo) ;
//ダメージ計算の計算式だけ訂正の余地あり。

Field* onEnemyTurn ( Field* pField ) ;      //敵の1ターンについて
Field* doEnemyAttack ( Field* pField ) ;       //敵の1回の攻撃について
int calcEnemyAttackDamage (Field* pField) ;
...
Field* spawnGems (Field* pField) ;      //スロットのEMPTYの箇所に宝石をランダム生成する
void GemsRelation (void) ;      //属性相性を表示する関数
void delayTime (double dTime) ;    // 処理に時間差を設ける関数。　※インクルード：<time.h>が必要。()内：送らせたい時間。
...
int goDungeon (Party* pParty, Dungeon* pDungeon){       //パーティ情報とDungoenを引き渡す。
    ...
    for (int i = 0 ; i <  pDungeon->numMonsters ; i+=1){
        printf("***ステージ%d***\n", i+1) ;
        knockCount += doBattle (pParty, &(pDungeon->pDungeonMonsters[i])) ;       //配列の特定要素をポインタで引き渡す→&( Pointa -> q[i] )　★難しい行
...
//味方の1ターンの攻撃に責任を持つ関数
Field* doAttack ( Field* pField,Element element,int Length ,int Combo) {
    int damage = calcAttackDamage(pField,element,Length ,Combo) ;
...
Field* doEnemyAttack ( Field* pField ) {
    ...
    if (pField->pParty->PartyHP <= 0){
        pField->pParty->PartyHP = 0;
    }
    return pField ;
}
...
void showBattleField (Field* pField){
    ...
    printf("\n") ;
    printf("----------------------------------------------------------\n") ;
    char Slot[14] = {'A','B','C','D','E','F','G','H','I','J','K','L','M','N'} ;        //アルファベットの表示
    for (int i=0; i<14 ; i+=1){
    printf("%c　",Slot[i]) ;
    }
...
Field* banishGems(Field* pField , BanishInfo* BanishPattern) {
    ...
    for (int i=0; i<4; i++){
        if (BanishPattern[i].element != EMPTY){
            ...
            pField->pParty->Combo += 1 ;
            printGems(pField->Slot) ;
            if ( BanishPattern[i].element != LIFE){
                doAttack(pField, BanishPattern[i].element, Length,pField->pParty->Combo) ;
            } else{
                doRecover(pField, Length, pField->pParty->Combo ) ;
            }
            if (pField->pParty->Combo >= 2) {
                printf ("　%dコンボ!!\n",pField->pParty->Combo) ;
                delayTime(0.3) ;
            }   else {printf ("\n") ;}
        }
...
int calcAttackDamage (Field* pField,Element element, int Length, int Combo) {
    //属性に応じたダメージ増減率を表す2次元配列。攻撃側→防御側とする
    double boost[4][4] ={}  ;
    ...
    for (int  i = 0; i<pField->pParty->PartyNumber;i++ ){
        if (pField->pParty->PartyMonsters[i].element == element){
            Monster AttackMonster = pField->pParty->PartyMonsters[i] ;
            break ;
        }
    }
    double AttackBoost = boost[element][pField->pEneMonster->element] ;
    int zyou = Length - 3.0 + Combo ;
    int damage = (int)( (AttackMonster.Attack*AttackBoost*2*pow(1.5,zyou) - pField->pEneMonster->Gard)*blurDamage(10)  ) ;         //****
    if (damage<=0){
        damage =1;
    }
    printf("\n") ;
    printMonsterName (&AttackMonster) ;
    printf("の攻撃！%dのダメージを与えた　",damage) ;
    delayTime(1.0) ;
    //printf("element番号：%d　AttackMonster：%s\n", element,AttackMonster.name) ;
    //printf("味方攻撃力：%d　敵防御力：%d　AttackBoost：%1.f",AttackMonster.Attack, pField->pEneMonster->Gard,AttackBoost) ;
    return damage;
}
...
Field* doRecover(Field* pField, int Length, int Combo) {
    int calcRHP = (int)(20*pow(1.5, Length -3+Combo)*3*blurDamage(10)) ;
    //printf("回復量:%d\n", calcRHP) ;
    int RecoverHP ;
    if ( pField->pParty->PartyHP + calcRHP <= 600) {
        RecoverHP = calcRHP ;
    }   else {
        RecoverHP = pField->pParty->PartyMaxHP - pField->pParty->PartyHP ;
    }
        printf("味方パーティのHPが%d回復した",RecoverHP) ;
        pField->pParty->PartyHP += RecoverHP ;
        delayTime(1.0) ;
    return pField;
}

int calcEnemyAttackDamage (Field* pField) {
    int damage = (int)( (pField->pEneMonster->Attack*1.5 - pField->pParty->PartyGard)*blurDamage(10) ) ;
    if (damage <= 0){
        damage =1;
    }
    return damage ;
...
Party organaizeParty (char* player, Monster* PartyMonsters, int PartyNumber) {
    ...
    Party party = {player, PartyMonsters, PartyNumber, sumHP, sumMaxHP, sumGard, 0} ;
    return party ;
}
...
void GemsRelation (void){
    printf("\n属性の相性：") ;
    printGem(WATER) ; printf(">  ") ; printGem(FIRE) ;  printf("   ") ;
    printGem(FIRE) ; printf(">  ") ; printGem(WIND) ;  printf("   ") ;
    printGem(WIND) ; printf(">  ") ; printGem(EARTH) ;  printf("   ") ;
    printGem(EARTH) ; printf(">  ") ; printGem(WATER) ;  printf("\n") ;
}
...
void delayTime (double dTime){
    ...
    do{
        time(&now) ;
    } while(difftime(now,start)<dTime) ;
    return;
}
