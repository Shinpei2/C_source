...
int calcAttackDamage (Field* pField,Element element, int Length, int Combo) {
    ...
    int damage = (int)( (AttackMonster.Attack*AttackBoost*(pow(1.5,zyou)*blurDamage(10)) - pField->pEneMonster->Gard) ) ;         //****
    if (damage<=0){
        damage =1;
    }
    printf("\n") ;
...
Field* doRecover(Field* pField, int Length, int Combo) {
    int calcRHP = 20*pow(1.5, Length -3+Combo)*blurDamage(10) ;
    int RecoverHP ;
...
int calcEnemyAttackDamage (Field* pField) {
    int damage = (int)( (pField->pEneMonster->Attack*blurDamage(10) - pField->pParty->PartyGard) ) ;
    if (damage <= 0){
        damage =1;
    }
    return damage ;
